...
EXPECTED_SIMPLE_ARTIST_COUNT = 3
EXPECTED_TIMED_ARTIST_COUNT = 3
EXPECTED_NEWLINE_ARTIST_COUNT = 2
EXPECTED_MULTILINE_ARTIST_COUNT = 2
...
def test_parse_artists_simple_list():
    ...
    assert len(artists) == EXPECTED_SIMPLE_ARTIST_COUNT
...
def test_parse_artists_with_set_times():
    ...
    assert len(artists) == EXPECTED_TIMED_ARTIST_COUNT
...
def test_parse_artists_handles_whitespace():
    ...
    assert len(artists) == EXPECTED_NEWLINE_ARTIST_COUNT
...
def test_parse_artists_with_ellipsis():
    ...
    assert len(artists) == EXPECTED_MULTILINE_ARTIST_COUNT
